    ...
    <div className="mt-8 pt-8 border-t border-card-border" data-testid="panel-evidence-drawer">
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="evidence" className="border-card-border">
          <AccordionTrigger className="text-muted-foreground hover:text-white">
          ...
          <AccordionContent className="space-y-4 pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-card p-4 rounded-md border border-card-border">
                <h4 className="font-medium text-white mb-2">Raw Timings (s)</h4>
              ...
              <div className="bg-card p-4 rounded-md border border-card-border">
                <h4 className="font-medium text-white mb-2">Modality Balance</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Visual Share</span>
                    <span className="text-blue-400">{(result.insights.modalityBalance.shares.visual * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Audio Share</span>
                    <span className="text-green-400">{(result.insights.modalityBalance.shares.audio * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Language Share</span>
                    <span className="text-rose-400">{(result.insights.modalityBalance.shares.language * 100).toFixed(1)}%</span>
                  </div>
                </div>
              </div>
            </div>
